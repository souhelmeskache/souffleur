...
|---|---|---|---|
| **checks=0** | `mapping-regles.json` portait `checks: 0` à CHAQUE passe (pconv0 → pconv3). `coderain/converter/aval.CHECK_RE` n'attendait que la forme `"DC N <ability> check"` ; le phrasé réel mesuré dans la source est `"<verbe> <compétence/caractéristique>[,] DC N"` (ex. formes : `"Make a perception check, DC N"`, `"roll survival, DC N"`, `"wisdom save, DC N"`) — DC systématiquement APRÈS le nom de compétence, jamais avant. | **résolu (D-254, Issue #77)** | `REVERSE_CHECK_RE` généralise la détection à l'ordre inverse (`<skill_or_ability>[,]? [check\|save\|saving throw\|roll]? DC N [check\|save\|saving throw\|roll]?`, au moins un mot-clé requis pour ne jamais capturer un DC de statblock nu) EN PLUS de `CHECK_RE` (forme existante inchangée, zéro régression). `SKILL_TO_ABILITY_5E` (`coderain/converter/aval.py`) route les 18 compétences SRD + `thieves' tools` vers leur caractéristique 5e — donnée versionnée, cohérente avec `ruletables.py`. `extract_checks()` fusionne les deux formes sans double-compte (chevauchement de span écarté). Primitive générique, pas une rustine DKS — tests `tests/test-check-re-ordre-inverse-d254.py` (fixtures synthétiques reprenant les formes exactes mesurées ci-dessus, zéro contenu narratif réel). **Re-passage sur `partition-pconv3` réelle (poste, `corpus_dir()`)** : `extract_checks` rejoué sur les 361 fichiers de nœuds — **51 jets détectés** (42 `check`, 9 `saving_throw`) contre 0 avant D-254 ; compte seul consigné ici, aucun contenu de module copié dans ce dépôt (D-109). Le régime de jet D-089 (facteurs A-F) n'est pas modifié : la détection alimente le régime proposé, elle ne touche pas le moteur. |
