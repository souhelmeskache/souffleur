...
if PCONV3.exists():
    ...
    if m_av:
        ...
        charniere = (fm_av.get("charniere_md", "")
                     ...
                     else av_text.split("## Charni├¿re de sortie")[-1].strip()
                     if "## Charni├¿re" in av_text else "")
    ...
    part_a.personnages.append(Personnage(
        ...
        destinee=[
            {"id": "jalon-origine",
             "intention_md": "Ancien soldat ayant surv├®cu ├á la guerre "
                             "d'Weathercote.",
...
try:
    ...
    for i in range(9):
        part_b.tensions.append(Tension(
            ...
            f"Description de la tension {i} ÔÇö pas de secret ici.",
            f"scene-{i}", [(i * 10, i * 10 + 5)]))
    part_b.secrets.append(Secret(
        "secret-pieux", "Le pieu de l'arbre rouge est la cl├® de tout.",
        "secret", ["scene-0"],
        {"declencheur": "inspection", "node_cible": "scene-0"},
        "R├®v├®ler brise l'immersion", [(0, 15)]))
...
try:
    ...
    part_d1.aventure = Aventure(
        [{"id": "traj-01", "description_md": "Qu├¬te",
...
print(f"\nOK test-pval-extension-conversation-b ÔÇö {len(FAIT)} sections vertes")
